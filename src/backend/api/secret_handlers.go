...
func (h *Handler) RotateSecret(w http.ResponseWriter, r *http.Request) {
	...
	if envCfg.Deployment == "swarm" {
		...
		if _, serr := h.bridge.EnsureSwarmSecret(name, env, body.Key, body.NewValue, newVer); serr != nil {
		...
		if err := workspace.SetSecretMeta(h.workspacesDir, name, env, envCfg.SecretKeys, versions); err != nil {
			writeJSON(w, http.StatusInternalServerError, map[string]string{"error": err.Error()})
			return
		}
		if err := h.bridge.Run(shellRun(name, env, "refresh", &out)); err != nil {
		...
		h.bridge.RemoveSwarmSecret(name, env, body.Key, oldVer) //nolint:errcheck
	} else {
		// Compose: update the plaintext value and restart affected services.
		if err := workspace.UpdateEnvVars(h.workspacesDir, name, env, map[string]string{body.Key: body.NewValue}, nil, nil); err != nil {
			writeJSON(w, http.StatusInternalServerError, map[string]string{"error": err.Error()})
			return
		}
		if _, _, perr := h.bridge.PushEnvFile(name, env); perr != nil {
			writeJSON(w, http.StatusBadGateway, map[string]string{"error": "saved but could not push to host: " + perr.Error()})
			return
		}
		if err := h.bridge.Run(shellRun(name, env, "restart", &out)); err != nil {
