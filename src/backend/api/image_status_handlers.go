...
func (h *Handler) TrackLatest(w http.ResponseWriter, r *http.Request) {
	...
	if cfgBytes, rerr := os.ReadFile(wspath.ConfigPath(h.workspacesDir, ws, name)); rerr == nil {
		...
		ro := composegen.RouteOpts{BaseDomain: settings.EffectiveBaseDomain(h.db, ws), AutoURLMode: settings.EffectiveAutoURLMode(h.db, ws), AutoURLHost: settings.EffectiveAutoURLHost(h.db, ws), Registry: cfg.Project.Registry, EnvFile: string(envContent)}
