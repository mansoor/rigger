...
// ── API Keys (external REST API credentials) ──────────────────────────────────

function ApiKeysTab() {
  const qc = useQueryClient()
  const { data: keys = [], isLoading } = useQuery({ queryKey: ['api-keys'], queryFn: fetchApiKeys })
  const { data: groups = [] } = useQuery({ queryKey: ['api-key-scopes'], queryFn: fetchApiKeyScopes })
  const { data: workspaces = [] } = useQuery({ queryKey: ['workspaces'], queryFn: fetchWorkspaces })
  const [modal, setModal]     = useState(false)
  const [revealed, setRevealed] = useState(null) // the just-created key (token shown once)
  const [deleting, setDeleting] = useState(null)

  const toggleMut = useMutation({
    mutationFn: ({ id, enabled }) => setApiKeyEnabled(id, enabled),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['api-keys'] }),
  })
  const delMut = useMutation({
    mutationFn: (id) => deleteApiKey(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['api-keys'] }); setDeleting(null) },
  })

  const opLabels = {}
  groups.forEach(g => g.ops.forEach(o => { opLabels[o.id] = o.label }))
  ...
  return (
    <div>
      ...
      {keys.length === 0 ? (
        <EmptyState icon="🔑" title="No API keys" description="Create a key to call the Rigger REST API from scripts or CI." action={<Btn onClick={() => setModal(true)}>＋ Create first key</Btn>} />
      ) : (
        <div className="space-y-2">
          {keys.map(k => (
            <div key={k.id} className={`flex items-center gap-4 p-4 bg-surface border border-border rounded-xl ${k.enabled ? '' : 'opacity-60'}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-content-subtle mt-0.5">
                  {k.project_access === 'specific' ? `${(k.projects || []).length} project(s)` : 'all projects'}
              ...
              <div className="flex items-center gap-2">
                <Btn variant="ghost" size="sm" onClick={() => toggleMut.mutate({ id: k.id, enabled: !k.enabled })}>{k.enabled ? 'Disable' : 'Enable'}</Btn>
                <Btn variant="danger" size="sm" onClick={() => setDeleting(k)}>Delete</Btn>
              </div>
            </div>
          ))}
        </div>
      ...
      {modal && (
        <CreateApiKeyModal
          ...
          onCreated={(k) => { setModal(false); setRevealed(k); qc.invalidateQueries({ queryKey: ['api-keys'] }) }}
        />
      )}
      {revealed && <RevealApiKeyModal apiKey={revealed} onClose={() => setRevealed(null)} />}
      {deleting && (
        <ConfirmDeleteModal name={`"${deleting.name}"`} onConfirm={() => delMut.mutate(deleting.id)} onClose={() => setDeleting(null)} loading={delMut.isPending} />
      )}
    </div>
  )
}

// CreateApiKeyModal: name + expandable scope groups (with per-op override) + project
// access (all/specific) + per-project rate limit + validity.
function CreateApiKeyModal({ groups, workspaces, onClose, onCreated }) {
  const [name, setName]           = useState('')
  const [scopes, setScopes]       = useState(() => new Set())
  const [expanded, setExpanded]   = useState(() => new Set())
  const [access, setAccess]       = useState('all')
  const [projects, setProjects]   = useState([]) // [{workspace, project}]
  ...
  const { data: pickProjects = [] } = useQuery({
    queryKey: ['projects', pickWs], queryFn: () => fetchProjects(pickWs), enabled: !!pickWs,
  })
  ...
  function addProject() {
    if (!pickWs) return
    setProjects(ps => ps.find(p => p.workspace === pickWs && p.project === '') ? ps : ps)
  }
  function addProjectRef(ws, proj) {
    setProjects(ps => ps.some(p => p.workspace === ws && p.project === proj) ? ps : [...ps, { workspace: ws, project: proj }])
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 backdrop-blur-sm overflow-y-auto py-8" onClick={onClose}>
      <div className="bg-surface border border-border rounded-xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="font-semibold text-content-strong">New API key</h3>
          <button onClick={onClose} className="text-content-subtle hover:text-content-strong text-xl">×</button>
        </div>
        ...
        <div className="space-y-4">
          ...
          {/* Scopes: expandable groups with per-operation override */}
          <div>
          ...
          <div>
            ...
            <div className="flex gap-3 mb-2">
              <label className="flex items-center gap-1.5 text-sm cursor-pointer"><input type="radio" checked={access === 'all'} onChange={() => setAccess('all')} className="accent-brand-500" /> All projects</label>
            ...
            {access === 'specific' && (
              <div className="space-y-2 border border-border-strong rounded-lg p-3">
                <div className="flex gap-2">
                  ...
                  <select disabled={!pickWs} onChange={e => { if (e.target.value) addProjectRef(pickWs, e.target.value); e.target.value = '' }} className={inputCls}>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {projects.map((p, i) => (
                      <span key={i} className="inline-flex items-center gap-1 text-xs bg-surface-raised border border-border-strong rounded px-2 py-0.5">
                        <span className="font-mono">{p.workspace}/{p.project}</span>
        ...
        <div className="flex justify-end gap-2 mt-6">
          <Btn variant="ghost" onClick={onClose}>Cancel</Btn>
          <Btn onClick={submit} disabled={createMut.isPending}>{createMut.isPending ? 'Creating…' : 'Create key'}</Btn>
        </div>
      </div>
    </div>
  )
...
// RevealApiKeyModal shows the raw token exactly once after creation.
function RevealApiKeyModal({ apiKey, onClose }) {
  const [copied, setCopied] = useState(false)
  function copy() {
    navigator.clipboard?.writeText(apiKey.token).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000) })
  }
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-surface border border-border rounded-xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex justify-end mt-5"><Btn onClick={onClose}>Done</Btn></div>
      </div>
    </div>
  )
}
